...
%% Intialization 
rng('default') ; 
key=433500 ;          % key for random number generators
key=key+eD ; 
rng(key) ; 
...
filename_xyz0=strcat('3DAS_MEMF_HTR_eD',num2str(eD),'nm_xyz0','.txt') ; 
save(filename_xyz0,'-ascii','xyz0') ;

%% Emitter activations
N=200 ;     % Temporal resolution (TR): N*Dt=2 sec
...
r00=1-Nape/((N-Nape)*(1+r21+r21*r32+r21*r32*r43)) ; % =0.9620
r10=1-r00 ; 
...
for n=1:N
  ...
  U16=uint16(U) ;
  filename_Frame=strcat('3DAS_MEMF_HTR_eD',num2str(eD),'nm_Frame',num2str(n),'.tif') ;
...
filename_Frame=strcat('3DAS_MEMF_HTR_eD',num2str(eD),'nm_Frame',num2str(10),'.tif') ;
...
filename_Frame=strcat('3DAS_MEMF_HTR_eD',num2str(eD),'nm_Frame',num2str(10),'.png') ;
...
% Average RMSMD-M: mean([8.82    8.69    9.20])
% Average RMSMD-F: mean([110.03  133.76  178.14])
