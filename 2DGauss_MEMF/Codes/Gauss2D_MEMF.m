...
filename_xy0=strcat('2DGauss_MEMF_eD',num2str(eD),'nm_xy0','.txt') ; 
...
for n=1:N
  ...
  U16=uint16(U) ;
  filename_Frame=strcat('2DGauss_MEMF_eD',num2str(eD),'nm_Frame',num2str(n),'.tif') ;
...
filename_Frame=strcat('2DGauss_MEMF_eD',num2str(eD),'nm_Frame',num2str(10),'.tif') ;
...
filename_Frame=strcat('2DGauss_MEMF_eD',num2str(eD),'nm_Frame',num2str(10),'.png') ;
...
%
  Figb=subplot(2,2,2) ;
  if Na(n)>0
    V=Gauss2D_Frame(sigma,Kx,Ky,Dx,Dy,Dt,Ih,b,mu,G,xyActive(:,1:Na(n),n)) ;
  else
    V=zeros(Ky,Kx) ;
  end
...
% Average RMSMD-M: mean([4.01 4.05 3.79])=3.95 (nm) 
% Average RMSMD-F: mean([15.97 26.11 75.16])=39.08 (nm)
