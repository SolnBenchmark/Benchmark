...
rp=Ih/b ;             % 180000
nup=beta*rp ; 
SPNR=10*log10(nup)    % 0.80 (dB)
rg=Ih/G ;             % 300000
nug=beta*rg ; 
SGNR=10*log10(nug)    % 3.02 (dB)
r=rp*rg/(rp+rg) ;     % 112500
nu=beta*r ; 
SNR=10*log10(nu)      % -1.24 (dB)
...
%
  ...
  if p==0, p=1 ; xyT(:,p)=xyFa(:,p) ; end   % avoid p=0
