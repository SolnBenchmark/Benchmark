%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SolnBenchmark: Gauss2D_MEMF_MTR 
% (1) Gaussian - PSF
...
filename_xy0=strcat('2DGauss_MEMF_MTR_eD',num2str(eD),'nm_xy0','.txt') ; 
...
Naae=(1-p0)*M ;           % =Nape*M/N=12, average # of activated emitters/frame
pd=1-(1-p0^N)^M ;         % =2.6504e-03, probability that at least one emitter 
...
for n=1:N
  ...
  U16=uint16(U) ;
  filename_Frame=strcat('2DGauss_MEMF_MTR_eD',num2str(eD),'nm_Frame',num2str(n),'.tif') ;
...
filename_Frame=strcat('2DGauss_MEMF_MTR_eD',num2str(eD),'nm_Frame',num2str(10),'.tif') ;
...
filename_Frame=strcat('2DGauss_MEMF_MTR_eD',num2str(eD),'nm_Frame',num2str(10),'.png') ;
...
% M=500; N=500; TR=5 sec
% pM: # of emitters activated at least once in a movie
% Distance:     40    30    20    Average (nm)
% pM:           500   500   500
% UGIA-M (nm):  3.19  3.36  3.30  3.28
% UGIA-F (nm):  15.57 18.42 37.80 23.93
% mean([15.57 18.42 37.80])
