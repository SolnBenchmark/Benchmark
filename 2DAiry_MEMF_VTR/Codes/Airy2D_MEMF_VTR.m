...
rp=Ih/b ;             % 120000
nup=beta*rp ; 
SPNR=10*log10(nup)    % -1.53 (dB)
rg=Ih/G ;             % 200000
nug=beta*rg ; 
SGNR=10*log10(nug)    % 0.69 (dB)
r=rp*rg/(rp+rg) ;     % 75000
nu=beta*r ; 
SNR=10*log10(nu)      % -3.57 (dB)
...
filename_xy0=strcat('2DAiry_MEMF_VTR_eD',num2str(eD),'nm_xy0','.txt') ; 
...
for n=1:N
  ...
  U16=uint16(U) ;
  filename_Frame=strcat('2DAiry_MEMF_VTR_eD',num2str(eD),'nm_Frame',num2str(n),'.tif') ;
...
filename_Frame=strcat('2DAiry_MEMF_VTR_eD',num2str(eD),'nm_Frame',num2str(10),'.tif') ;
...
filename_Frame=strcat('2DAiry_MEMF_VTR_eD',num2str(eD),'nm_Frame',num2str(10),'.png') ;
...
% pM:           490   490   492
% UGIA-M (nm):  4.45  3.81  2.83    3.70
% UGIA-F (nm):  25.13 70.48 172.39  89.33
% mean([25.13 70.48 172.39]) 
