...
% 'high SNR'          % r=1200000, SNR=60.79 (dB)
b=0.3 ;               % rp=2000000, SPNR=63.01 (dB)
G=0.2 ;               % rg=3000000, SGNR=64.77 (dB)
...
filename_xyz0=strcat('3DAS_MEMF_VTR_eD',num2str(eD),'nm_xyz0','.txt') ; 
...
for n=1:N
  ...
  U16=uint16(U) ;
  filename_Frame=strcat('3DAS_MEMF_VTR_eD',num2str(eD),'nm_Frame',num2str(n),'.tif') ;
...
filename_Frame=strcat('3DAS_MEMF_VTR_eD',num2str(eD),'nm_Frame',num2str(10),'.tif') ;
...
filename_Frame=strcat('3DAS_MEMF_VTR_eD',num2str(eD),'nm_Frame',num2str(10),'.png') ;
...
axis([Dt Dt*N min([min(RMSMD_M) min(RMSMD_F)]) max([max(RMSMD_M) max(RMSMD_F)])]) ;
ylabel('RMSMD (nm)') ; 
...
% pM:           491     495     495
% UGIA-M (nm):  6.93    6.73    7.57    7.08
% UGIA-F (nm):  139.77  182.91  248.23  190.30
% mean([139.77  182.91  248.23]) 
