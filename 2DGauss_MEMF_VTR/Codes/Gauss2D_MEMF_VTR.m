...
% 'high SNR'          % r=75000, SNR=48.75 (dB)
b=5 ;                 % rp=120000, SPNR=50.79 (dB)
G=3 ;                 % rg=200000, SGNR=53.01 (dB)
...
M=500 ; 
phi=0.1 ; beta=7+0.5*rand ;  % helix parameters 
xy1=zeros(2,M) ; 
t=zeros(1,M) ; 
...
filename_xy0=strcat('2DGauss_MEMF_VTR_eD',num2str(eD),'nm_xy0','.txt') ; 
save(filename_xy0,'-ascii','xy0') ;

%% Emitter activations
N=50 ;      % Temporal resolution (TR): N*Dt=0.5 sec
Nape=12/2 ; % Average # of activations per emitter in data movie
...
p4=r10*r21*r32*r43/den ;  % =0.0021, probability of state 4
pa=1-p0 ;                 % =0.1200, probability of activation
Naae=(1-p0)*M ;           % =Nape*M/N=60, average # of activated emitters/frame
pd=1-(1-p0^N)^M ;         % =0.5676, probability that at least one emitter
...
for n=1:N
  ...
  U=Gauss2D_Frame(sigma,Kx,Ky,Dx,Dy,Dt,Ih,b,mu,G,xyActive(:,1:p,n)) ;
  U16=uint16(U) ;
  filename_Frame=strcat('2DGauss_MEMF_VTR_eD',num2str(eD),'nm_Frame',num2str(n),'.tif') ;
...
filename_Frame=strcat('2DGauss_MEMF_VTR_eD',num2str(eD),'nm_Frame',num2str(10),'.tif') ;
...
filename_Frame=strcat('2DGauss_MEMF_VTR_eD',num2str(eD),'nm_Frame',num2str(10),'.png') ;
...
% M=500; N=50; TR=0.5 sec
% pM: # of emitters activated at least once in a movie
% Distance:     40    30    20      Average (nm)
% pM:           492   491   495
% UGIA-M (nm):  4.25  3.67  2.92    3.61
% UGIA-F (nm):  38.06 95.97 170.28  101.44
% mean([38.06 95.97 170.28]) 
